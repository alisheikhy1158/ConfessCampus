...
const sendMessage = async (req, res) => {
    try {
        // chatId can come from the URL param or the request body (URL preferred)
...
const deleteMessage = async (req, res) => {
    try {
        ...
        if (message.sender.toString() !== userId) {
            return res.status(403).json({ success: false, message: 'Unauthorized - Can only delete your own messages' });
        }
